...
Classic snake game where speed increases as items are collected and added to the snake length. It supports keyboard or button controls (on mobile). Make use of `<canvas>` for game logic and play through.

### Combo Breaker

As a fan of fighting games (definitely not good at them though), I thought this would be a neat little game to make. It asks players to put in the inputs of various fighting games franchises (Tekken, Mortal Kombat, Soul Calibur, etc). It also contains USB controller support as well.

### SoundWave

As an avid rhythm game player, I wanted to make a small game that would emulate rhythm games. I found some free music to use and was able to create a script that would automatically put points on the "music map". It's definitely not perfect and needs a lot of refinement for each one, but it does work!
